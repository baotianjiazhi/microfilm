...
class Movie(db.Model):
    ...
    area = db.Column(db.String(255))#上映地点
...
class Comment(db.Model):
    ...
    movie_id = db.Column(db.Integer, db.ForeignKey('movie.id'))#所属用户
...
class Admin(db.Model):
    __tablename__ = "admin01"
    id = db.Column(db.Integer, primary_key=True)  # 编号
    name = db.Column(db.String(100), unique=True)  # 管理员账号
    pwd = db.Column(db.String(100)) #管理员密码
    is_super = db.Column(db.SmallInteger) #是否为超级管理员，定义0为超级管理员
    role_id = db.Column(db.Integer, db.ForeignKey('role.id')) #所属角色
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    adminlogs = db.relationship("Adminlog", backref='admin01')# 管理员登陆日志
    oplogs = db.relationship("Oplog", backref='admin01')# 管理员操作日志
...
class Adminlog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    admin_id = db.Column(db.Integer, db.ForeignKey('admin01.id'))  # 所属管理员
    ip = db.Column(db.String(100))  # 登陆Ip
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 登陆时间
